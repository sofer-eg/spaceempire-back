...
(10.20 L2; `alwaysVisibleStatic` в `sector/state.go` пропускает без гейта только
станции/верфи/ТС/пирбазы, а башни, спутники, джаммеры **и ворота** —
радарно-гейтятся), приезжает в `staticsAdded` только раскладкой: живой записи в
патче нет, а из `staticCombat` объект вылетел ещё при выходе из окна
(`staticsRemoved`). До следующего dirty-события клиент показывает не спавновое
число, а **«Состояние цели недоступно.»** — и шит-бар над объектом на канвасе
пропадает. То есть это честная деградация «нет данных», а не врущий UI: показ
спавнового числа как текущего — ровно то, что откатили в TASK-166, и он тут не
воспроизводится. Тот же корень, другой триггер; welcome/реконнект/прыжок закрыты.
