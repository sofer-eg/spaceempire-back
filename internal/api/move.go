...
func (s *Server) handleMove(w http.ResponseWriter, r *http.Request) {
	...
	// PlayerID is set by RequireAuth; tests without auth get zero, which
	// matches a ship's default PlayerID — so legacy unit tests still pass.
	playerID, _ := auth.PlayerIDFromContext(r.Context())

	var targetRef *domain.EntityRef
	...
	err := s.router.Send(currentSector, sector.MoveCommand{
		PlayerID:  playerID,
		ShipID:    domain.ShipID(req.ShipID),
		Target:    domain.Vec2{X: req.X, Y: req.Y},
		TargetRef: targetRef,
		Reply:     reply,
	})
