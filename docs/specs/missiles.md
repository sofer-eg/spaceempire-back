...
- **Поражаемость** — `domain.Container.TakeDamage` (щита нет), HP **только в
  RAM**: колонки нет, `Config.ContainerHP` (дефолт 25 — ниже 30 урона ракеты, то
  есть одно попадание уничтожает) штампуется на cold-start и в
...
| `launch-drone` | 21 (TASK-167 свёл дубликат 51 на каталожный товар) | размер залпа считается ВНУТРИ транзакции -- `min(cap up_drone_control - живые, остаток в трюме)` (TASK-176) -- затем списание этого числа + столько же `dronesRepo...Create`. Короткий трюм укорачивает залп, пустой -- отказ 400 (см. `drones.md` §2.1) |
