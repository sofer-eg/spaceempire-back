...
- **Дедупликация.** Реф может быть одновременно в `addedRefs` и в dirty-наборе
  ...
  запись.
- **Реф без живой записи пропускается**, а не роняет тик. Ветка защитная, а не
  рабочая: рефы, приходящие сюда из `broadcastPatches`, происходят из
  `visibleStaticRefs`, которая идёт по `s.destructibles`, — то есть сегодня она
  сработать не может. Оставлена, чтобы хелпер не стал ловушкой для будущего
  вызывающего кода, собирающего рефы иначе.
