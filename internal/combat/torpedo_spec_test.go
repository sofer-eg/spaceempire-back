...
// two torpedo classes rather than literal magnitudes (a balance decision, C-01):
// each class hits far harder than a missile, is slower and less nimble, carries
// area splash and its own finite HP, and class 3 dominates class 2 on every axis.
func TestUnit_TorpedoSpecs_Profile(t *testing.T) {
	t.Parallel()

	c2, ok := torpedoSpecsByClass[2]
	require.True(t, ok, "class 2 torpedo spec must exist")
	c3, ok := torpedoSpecsByClass[3]
	require.True(t, ok, "class 3 torpedo spec must exist")

	mis := DefaultMissileSpec()
	for name, s := range map[string]TorpedoSpec{"class2": c2, "class3": c3} {
